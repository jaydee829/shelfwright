...
- **Notes**:
  ...
  - **#69** (OPEN, dry-run validated) — corrected the prune distinguisher to genre/mood membership after a prod dry-run caught it deleting real tropes (bugs.md 2026-06-24). **NEXT operator steps (prod, via proxy): merge #69 → deploy → `--prune-fallbacks --apply --yes` (~394 links/123 works) → `--tropes` (dry-run → apply).**
