...
- **Notes**:
    - **Pending live verification (quota-gated)**: (1) subagent visibility of the in-process librarian MCP server (`AgentDefinition.mcpServers=["librarian"]`, REC-019 pattern; fallback = rescope tools onto the Librarian); (2) explorer subagent's `WebSearch` tool name; (3) 2-turn live smoke per backend (`test/integration/test_cli_chat_live.py`, api_dependent); (4) MLflow connect at CLI startup is fast (a hung — not refused — tracking server would block the banner; escape hatch `--no-mlflow`).
