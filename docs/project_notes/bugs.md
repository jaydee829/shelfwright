...
### 2026-06-23 - Bulk import leaves a genre/mood-as-trope fallback layer that single-pass enrichment doesn't (OPEN — backend bench)
...
- **Solution**: OPEN — backend bench owns next. Two candidates: **(1)** have the two-phase deep pass DELETE the work's `justification IS NULL` fallback rows once it persists real tropes — simple, but it strips the genre/mood matching signal (works have no embedding; genres/moods reach matching ONLY via these fallback rows — the work-representation gap), trading UI tidiness for matching signal; **(2)** the deferred **work-representation refactor** (give `Work` its own genre/mood representation, then drop genre-as-trope fallbacks entirely) — the proper fix. Decide (1) vs (2) with the representation gap in mind. Investigation tool + queries preserved at `scripts/trope_audit.py` on branch `feat/import_auditing`.
