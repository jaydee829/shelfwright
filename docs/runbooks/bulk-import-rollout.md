...
it the same way as the Lift 1 multi-user migration (`docs/runbooks/lift1-multi-user-rollout.md`):
via the Cloud SQL Auth Proxy, routing through the **app container** (bare WSL `python` has no
`sqlalchemy`/`alembic`). The prod DB credentials are **not** in any local file — they live in Secret
Manager as `librarian-db-url`; pull the whole URL and rewrite only the host.

> **Prerequisite:** run this from a WSL clone that actually contains the migration file — it only
> exists on the PR branch. Either `git checkout` the bulk-import branch first, or merge to `main` and
> `git pull` before running (the deploy→migrate gap only affects the brand-new, unused import tables).
> The migration is additive (two new tables + indexes), so it is safe to run while the service is live.
...
```bash
docker run --rm -v "$PWD":/app -w /app --add-host=host.docker.internal:host-gateway \
  -e DATABASE_URL="$PROD_DB_URL" agentic_librarian-app:latest alembic current   # → 7b7b4d6ae6f6 (head)
```

(Unlike Lift 1, no `alembic stamp` is needed — prod is already tracked at `30f1e46533e9`, so `upgrade
head` just applies the one new revision on top.)
