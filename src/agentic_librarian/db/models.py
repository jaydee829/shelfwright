...
from sqlalchemy import ARRAY, Column, Date, Float, ForeignKey, Integer, String, Table, Text, DateTime
...
class Author(Base):
    ...
    style_attributes: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
    
    works: Mapped[List["Work"]] = relationship(
        secondary=work_contributors, back_populates="authors"
    )
...
class Work(Base):
    ...
    moods: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)
    
    authors: Mapped[List["Author"]] = relationship(
        secondary=work_contributors, back_populates="works"
    )
...
class Edition(Base):
    ...
    isbn_13: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    format: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    page_count: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    audio_minutes: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
...
class ReadingHistory(Base):
    ...
    date_started: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
...
class Suggestions(Base):
    ...
    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4, nullable=False)
    work_id: Mapped[UUID] = mapped_column(ForeignKey("works.id"), nullable=False)
    suggested_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
